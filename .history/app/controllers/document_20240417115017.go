...
func (this *DataController) InsertCompressRoot(folder string) (err error) {
	documentId := this.GetString("document_id", "")
	if documentId == "" {
		this.Abort("页面参数错误！")
	}

	doc, err := models.DocumentModel.GetDocumentByDocumentId(documentId)
	if err != nil {
	...
	// 获取文件目录绝对路径
	doc := map[string]string{
		"parent_id": doc.parentId, "space_id": info.spaceId, "name": "", "type": "1",
		"path": doc.path + "," + info.parentId,
	}
	_, pageFile, _ := models.DocumentModel.GetParentDocumentsByDocument(doc)
	absFilePath := utils.Document.GetAbsPageFileByPageFile(pageFile)
	folder, _ := filepath.Split(absFilePath)
