...
func (this *DataController) DecompressToDoc() {
	...
	/*
		...
		pageFolder = filepath.Join(pageFolder, c.folder)
	...
	*/
	...
	this.InsertCompressRoot(c.fileRoot)
...
func (this *DataController) InsertCompressRoot(root string) (res int) {
	documentId := this.GetString("document_id", "")
	if documentId == "" {
		this.Abort("页面参数错误！")
	}
	...
	res, err = models.DocumentModel.Insert(insertDocument)
	if err != nil {
		this.Abort("创建文档失败：" + err.Error())
	}

	logs.Info(newId)

	this.Data["json"] = insertDocument
